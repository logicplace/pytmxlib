...
def from_dict_method(func):
	...
	def _wrapped(cls, dct, *args, **kwargs):
		...
		if dct:
			...
			raise ValueError(message.format(cls.__name__, ', '.join(str(k) for k in dct.keys())))
		return result
...
def assert_item(dct, key, expected_value):
	...
	if actual_value != expected_value:
		raise ValueError(
			'bad value: {0} = {1}; should be {2}'.format(key, actual_value, expected_value)
		)
...
class NamedElementList(collections.abc.MutableSequence):
	...
	def __setitem__(self, index_or_name, value):
		...
		with self.modification_context():
			if isinstance(index_or_name, slice):
				self.list[index_or_name] = (self.stored_value(i) for i in value)
			else:
	...
	def __getitem__(self, index_or_name):
        ...
        """
		if isinstance(index_or_name, slice):
			return [self.retrieved_value(item) for item in self.list[index_or_name]]
		else:
	...
	def append(self, value):
		with self.modification_context():
			self.list.append(self.stored_value(value))
